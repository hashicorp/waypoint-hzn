...
var (
	...
	DefaultDBName = "waypoint_hzn_test"
	...
	UserName     = "waypoint_hzn_testuser"
...
var gormDebug = flag.Bool("waypoint-hzn-gorm-debug", false, "set to true to have Gorm log all generated SQL.")
...
func testMigrate(t testing.T, family, dbName string) {
	...
	driver, err := migratePostgres.WithInstance(db.DB(), &migratePostgres.Config{
		MigrationsTable: "waypoint_hzn_test_migrations",
	})
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func gomodDir(t testing.T) string {
	...
	for _, n := range names {
		if strings.HasPrefix(n, "waypoint-hzn@") {
